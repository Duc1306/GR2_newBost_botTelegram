...
def test_me(token, expected_role):
...
def test_admin_endpoint(token, is_admin):
...
if __name__ == "__main__":
    ...
    user_token = test_login("user", "user123", "user")
    test_bad_login()
    ...
    if user_token:
        test_me(user_token, "user")

    print("\n3. Admin endpoint access control:")
    if admin_token:
        print("  Admin token:")
        test_admin_endpoint(admin_token, is_admin=True)
    if user_token:
        print("  User token (should 403):")
        test_admin_endpoint(user_token, is_admin=False)
